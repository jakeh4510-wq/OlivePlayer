 ...
 *
 * Movies array now uses embed URLs for all movies to avoid black screens.
 */
...
const MOVIES = [
  ...
  { name: "A Father for Brittany (1998)", url: "https://hyhd.org/embed/195406/", directUrl: null },
  { name: "Inspector Zende (2025)", url: "https://hyhd.org/embed/195389/", directUrl: null },
...
export default function OlivePlayer() {
  ...
  useEffect(() => {
    fetch(PLAYLISTS.live)
      ...
      .then((text) => {
        ...
        if (live.length) setCurrentStreamUrl(live[0].url);
      })
  ...
  useEffect(() => {
    if (!vjsPlayer.current && videoRef.current) {
      ...
      vjsPlayer.current.on("error", () => console.warn("video.js player error:", vjsPlayer.current.error()));
    }
    return () => {
  ...
  // Update video.js when currentStreamUrl changes
  useEffect(() => {
  ...
  const playMovie = (movie) => {
    ...
    if (iframeRef.current) {
      iframeRef.current.style.display = "none";
      setTimeout(() => {
        iframeRef.current.src = movie.url; // <-- Embed URL used here
      }, 50);
    }
  };
        ...
        <div style={{ width: "95%", maxWidth: 1400, height: 700, display: "flex", justifyContent: "center", alignItems: "center" }}>
          ...
          {section === "movies" && currentMovie && !currentMovie.directUrl && (
            <div style={{ position: "relative", width: "100%", height: "100%" }}>
              <div ref={loaderRef} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", fontSize: 20, color: "#fff", zIndex: 2 }}>Loading...</div>
              <iframe
                ref={iframeRef}
